...
void jni_load(void) {
  ...
  *(uintptr_t *)(fake_vm + 0x18) = (uintptr_t)GetEnv;
  ...
  int (* JNI_OnLoad)(void *vm, void *reserved) = (void *)so_symbol(&bc2_mod, "JNI_OnLoad");
  JNI_OnLoad(fake_vm, NULL);
  memset(fake_app, 'A', sizeof(fake_app));
  int (* android_main)(void* app) = (void *)so_symbol(&bc2_mod, "android_main");
  // android_main(fake_app);
  // void (* low_shaders)(bool low) = (void *)so_symbol(&bc2_mod, "_ZN9wfShaders16SetUseLowQualityEb");
  // low_shaders(false);
  void (* fmod_foreground)(void) = (void *)so_symbol(&bc2_mod, "NCT_AndroidFmodForeground");
  fmod_foreground();
  // void (* foreground)(bool low) = (void *)so_symbol(&bc2_mod, "_ZN19wfAndroidDeviceGlue15SetIsForegroundEb");
  // foreground(true);
  printf("call android_main\n");
  android_main(fake_app);
